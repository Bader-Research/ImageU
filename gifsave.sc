 ...
 *
 */
static int InputByte(int v, int w, int q, int r, int image[v][w]::[q][r],
		     int (*pixmap)())
{
    int ret;
    register int
    ...
    ret = pixmap(image[i][j][ii][jj],v*q,r*w);
 ...
 *
 */
static int LZW_Compress(int codesize, int v, int w, int q, int r,
			int image[v][w]::[q][r],
			int (*pixmap)())
{
    register int c;
     ...
     *  Pack image
     */
    while ((c = InputByte(v,w,q,r,image,pixmap)) != -1) {
        /*
		      ...
		      int (*pixmap)())
{
    ...
    if ((errcode = LZW_Compress(codesize, v, w, q, r, image, pixmap))
	!= GIF_OK)
        return errcode;
